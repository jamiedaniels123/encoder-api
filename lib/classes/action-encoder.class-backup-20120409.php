...
//				error_log($function." -- ".$retData['result']);
	...
	function doEncoderPushToMedia($mArr,$mNum,$cqIndex,$step)
	{
		...
		error_log("doEncoderPushToMedia | ".$mArr." | ".$mNum." | ".$cqIndex." | ".$step." | ".$sourceFilePath." | ".$outFile);
    if (file_exists($sourceFilePath)) {
      ...
      // been removed from the output
			chmod($sourceFilePath, 0664);  // BH 2010921 - changed permissions to be 664 rather than 665 which is wrong
			...
			if ($retData['scp'][0]==0) {
				...
				//error_log("doEncoderPushToMedia | SELECT cq_filename FROM queue_commands WHERE cq_filename = '".$mArr['source_filename']."' AND cq_status='N' AND cq_step=".$step." ");
				//error_log("doEncoderPushToMedia | ".$mArr['source_filename']." | Number of rows in database ".$result0->num_rows);
				if ($result0->num_rows==1) {
					// BH 20120402 this doesn't appear to be working
					error_log("doEncoderPushToMedia | Attempting to delete file: ".$sourceFilePath);
					if (unlink($destination['encoder-output'].$inFile)) {
						error_log("doEncoderPushToMedia | Failed to delete file");
					} else {
						error_log("doEncoderPushToMedia | Successfully delete file");
					}
				}
			} else {
				$retData['result']='F';
